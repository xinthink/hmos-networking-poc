...
4. **HTTPS 自签证书**：Cangjie 只有 `caPath`（文件路径），无 `caData`（内存 PEM）。
   `CA_PATH = /data/storage/el1/bundle/entry/resources/resfile/mock-ca/cert.pem`
   （bundle 只读路径，证书必须随 `resfile/mock-ca/` 打包，见跨工程约定 2）。
5. **multipart 必须显式设 `Content-Type: multipart/form-data` header**，否则 Network
   Kit 不进入 multipart 模式（server 收到 partCount=0）。
6. **Cangjie 语法**：`Byte` 就是 `UInt8`（typealias）；数值转换用类型构造函数
...
7. **Cangjie 无 JSON 库**：不要假设有 `JSON.parse`；用工程内极简提取器。
8. **UI 自动化读文本会被截断**：`uitest dumpLayout` 导出的 `text` 字段对长文本只截
